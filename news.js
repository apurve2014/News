	...
	xmlhttp.onreadystatechange = () => {
		if (xmlhttp.readyState == 4) {
			...
			if (news.status == "ok" && news.allCountries) {
				console.log("totle Countries : ", news.allCountries.length);
...
function showingAllCountries(news) {
	...
	news.forEach((element, index) => {
		...
		if (tdCount.length === intr) {
			...
			if (tdCount[tdCount.length - 1] === news.length - 1) {
				console.log("Hi :", tdCount[tdCount.length - 1]);
				let tbltr = `<tr>${tableTds}</tr>`;
				tableTrs += tbltr;
			}
		}
